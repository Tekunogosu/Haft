...
namespace Toolsmith.Compat {
    ...
    //Everything Toolsmith needs to know about ScientificSmithy lives here, so the rest of the codebase can ask for a
    ...
    //result can get, and where it is absent Toolsmith's own config multiplier does.
    public static class ScientificSmithyCompat {
        ...
        public static int CalculateMaxSharpness(ItemStack statsSource, int baseDurability) {
            ...
            return (int)(baseDurability * ToolsmithModSystem.Config.SharpnessMult);
        }
