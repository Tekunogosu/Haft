...
namespace Haft {
    public class HaftModSystem : ModSystem {
        ...
        public static HaftConfigs Config;
        public static HaftClientConfigs ClientConfig; //Any setting in here is NOT synced from the server, and intended to control client-side changes only.
        public static HaftPartStats Stats;
        ...
        public override void Start(ICoreAPI api) {
            ...
            if (!ClientConfig.DisableMultiPartRendering) {
                api.World.Config.SetBool(HaftConstants.DisabledMultiPartRenders, true);
            } else {
                api.World.Config.SetBool(HaftConstants.DisabledMultiPartRenders, false);
            }

            //This is important to let the Treasure Hunter Trader accept a Haft Pick to get the map for Story Content! Thank you Item Rarity for also having the issue and both leaving a comment in their code and pushing the commit not too long before I had the same problem :P
            if (!IgnoreAttributesAdded) { //This bool is entirely for checking if it's singleplayer or not, since we don't want to run it twice if it is. That'll just double up the attributes in the list.
                ...
                string[] ignoreTheseAttributes = new string[globalLen + HaftAttributes.HaftIgnoreAttributesArray.Length];
                for (int i = 0; i < globalLen; i++) {
                    ignoreTheseAttributes[i] = GlobalConstants.IgnoredStackAttributes[i];
                }
                for (int i = globalLen; i < HaftAttributes.HaftIgnoreAttributesArray.Length + globalLen; i++) {
                    ignoreTheseAttributes[i] = HaftAttributes.HaftIgnoreAttributesArray[i - globalLen];
                }
        ...
        public override void StartClientSide(ICoreClientAPI api) {
            ...
            string configBase64String = api.World.Config.GetString(HaftConstants.HaftConfigKey, "");
            if (configBase64String != "") {
                try {
                    ...
                    Config = JsonConvert.DeserializeObject<HaftConfigs>(configJson);
            ...
            string statsBase64String = api.World.Config.GetString(HaftConstants.HaftStatsKey, "");
            if (statsBase64String != "") {
                try {
                    ...
                    Stats = JsonConvert.DeserializeObject<HaftPartStats>(statsJson);
            ...
            string woodInBindingBase64String = api.World.Config.GetString(HaftConstants.HaftWoodInToolBindingsData, "");
        ...
        public override void AssetsLoaded(ICoreAPI api) {
            ...
            string[] allTags = ["haft-part", "haft-maintenance", "haft-binding", "haft-handle", "haft-head"];
        ...
        private void ProcessJsonPartsAndStats(ICoreAPI api) {
            if (Config.EnableEditsForRegex) {
                Logger.Debug("Server is starting with Config Edits for the Regex Strings enabled. If something goes wrong, the changes made could be the cause. Disable the option to reset configs to the generated default, or if you want to update the defaults from the JSON files again - it will not update from any compatability files while this is active. If you report an issue with this enabled, please include your Haft.json config changes as well as the logs!");
            } else {
                Config.ToolHeads = BuildRegexFromAssets(api, "config/haft/regex/toolheads", ConfigUtility.AnyWildcardStringPrefix);
                Config.TinkerableTools = BuildRegexFromAssets(api, "config/haft/regex/tinkerabletools", ConfigUtility.AnyFirstCodeStartStringPrefix);
                Config.SinglePartTools = BuildRegexFromAssets(api, "config/haft/regex/singleparttools", ConfigUtility.AnyFirstCodeStartStringPrefix);
                Config.BluntHeadedTools = BuildRegexFromAssets(api, "config/haft/regex/bluntheadedtools", ConfigUtility.AnyFirstCodeStartStringPrefix);
                Config.PartBlacklist = BuildRegexFromAssets(api, "config/haft/regex/partblacklist", ConfigUtility.AnyWildcardStringPrefix);
                Config.ToolsWithWoodInBindingShape = BuildRegexFromAssets(api, "config/haft/regex/woodinbindingshapes", ConfigUtility.AnyWildcardStringPrefix);
            }

            if (Stats.EnableEdits) {
                Logger.Debug("Server is starting with Config Edits for the Parts and Stats enabled. If something goes wrong, the changes made could be the cause. Disable the option to reset configs to the generated default. If you report an issue with this enabled, please include your HaftPartsStats.json config changes as well as the logs!");
            }

            if (Config.RunFullJsonVerifying) {
                Logger.Debug("Running full Json verification for all found Haft Configs for parts and stats.");
            }

            Dictionary<AssetLocation, List<HandlePartDefines>> handleParts = api.Assets.GetMany<List<HandlePartDefines>>(api.Logger, "config/haft/parts/handles");
            foreach (var handlePart in handleParts) {
                HaftPartStatsHelpers.VerifyAndStoreDefinesInDict(handlePart.Value, Config.RunFullJsonVerifying, ref Stats.BaseHandleParts);
            }

            Dictionary<AssetLocation, List<GripPartDefines>> gripParts = api.Assets.GetMany<List<GripPartDefines>>(api.Logger, "config/haft/parts/grips");
            foreach (var gripPart in gripParts) {
                HaftPartStatsHelpers.VerifyAndStoreDefinesInDict(gripPart.Value, Config.RunFullJsonVerifying, ref Stats.GripParts);
            }

            Dictionary<AssetLocation, List<TreatmentPartDefines>> treatmentParts = api.Assets.GetMany<List<TreatmentPartDefines>>(api.Logger, "config/haft/parts/treatments");
            foreach (var treatmentPart in treatmentParts) {
                HaftPartStatsHelpers.VerifyAndStoreDefinesInDict(treatmentPart.Value, Config.RunFullJsonVerifying, ref Stats.TreatmentParts);
            }

            Dictionary<AssetLocation, List<BindingPartDefines>> bindingParts = api.Assets.GetMany<List<BindingPartDefines>>(api.Logger, "config/haft/parts/bindings");
            foreach (var bindingPart in bindingParts) {
                HaftPartStatsHelpers.VerifyAndStoreDefinesInDict(bindingPart.Value, Config.RunFullJsonVerifying, ref Stats.BindingParts);
            }

            Dictionary<AssetLocation, List<HandleStatDefines>> handleStats = api.Assets.GetMany<List<HandleStatDefines>>(api.Logger, "config/haft/stats/handles");
            foreach (var handleStat in handleStats) {
                HaftPartStatsHelpers.VerifyAndStoreDefinesInDict(handleStat.Value, Config.RunFullJsonVerifying, ref Stats.BaseHandleStats);
            }

            Dictionary<AssetLocation, List<GripStatDefines>> gripStats = api.Assets.GetMany<List<GripStatDefines>>(api.Logger, "config/haft/stats/grips");
            foreach (var gripStat in gripStats) {
                HaftPartStatsHelpers.VerifyAndStoreDefinesInDict(gripStat.Value, Config.RunFullJsonVerifying, ref Stats.GripStats);
            }

            Dictionary<AssetLocation, List<TreatmentStatDefines>> treatmentStats = api.Assets.GetMany<List<TreatmentStatDefines>>(api.Logger, "config/haft/stats/treatments");
            foreach (var treatmentStat in treatmentStats) {
                HaftPartStatsHelpers.VerifyAndStoreDefinesInDict(treatmentStat.Value, Config.RunFullJsonVerifying, ref Stats.TreatmentStats);
            }

            Dictionary<AssetLocation, List<BindingStatDefines>> bindingStats = api.Assets.GetMany<List<BindingStatDefines>>(api.Logger, "config/haft/stats/bindings");
            foreach (var bindingStat in bindingStats) {
                HaftPartStatsHelpers.VerifyAndStoreDefinesInDict(bindingStat.Value, Config.RunFullJsonVerifying, ref Stats.BindingStats);
            }
            ...
            Dictionary<AssetLocation, List<MaterialStatDefines>> materialStats = api.Assets.GetMany<List<MaterialStatDefines>>(api.Logger, "config/haft/stats/materials");
            foreach (var materialStat in materialStats) {
                HaftPartStatsHelpers.VerifyAndStoreDefinesInDict(materialStat.Value, Config.RunFullJsonVerifying, ref Stats.MaterialStats);
            }
            ...
            SaveToWorldData(api, HaftConstants.HaftConfigKey, Config);
            SaveToWorldData(api, HaftConstants.HaftStatsKey, Stats);
        ...
        private void VerifyMaterialsAreLoaded() {
            ...
            if (missing.Count > 0) {
                Logger.Error("No MaterialStatDefine was loaded for: " + string.Join(", ", missing) +
                    ". Handles of those materials will fall back to " + HaftConstants.DefaultMaterialStatKey +
                    " and be given the wrong durability. Check that config/haft/stats/materials and " +
                    "config/haft/stats/woods contain an entry for each.");
            } else {
        ...
        private void TryToLoadConfig(ICoreAPI api) { //Created following the tutorial on the Wiki!
            try {
                Config = api.LoadModConfig<HaftConfigs>(ConfigUtility.ConfigFilename);
                if (Config == null) {
                    Config = new HaftConfigs();
                } else {
                    ...
                    if (!DoesConfigNeedRegen && ConfigUtility.ConfigKeysDifferFromDefaults<HaftConfigs>(api, ConfigUtility.ConfigFilename)) {
                    ...
                    if (!DoesConfigNeedRegen && !Config.EnableEditsForRegex) {
                        HaftConfigsHelpers.ResetRegexStrings(ref Config);
                    }
                }
        ...
        private void TryToLoadClientConfig(ICoreAPI api) {
            try {
                ClientConfig = api.LoadModConfig<HaftClientConfigs>(ConfigUtility.ClientConfigFilename);
                if (ClientConfig == null || DoesConfigNeedRegen) {
                    ClientConfig = new HaftClientConfigs();
                }
        ...
        private void TryToLoadStats(ICoreAPI api) {
            try {
                Stats = api.LoadModConfig<HaftPartStats>(ConfigUtility.StatsFilename);
                if (Stats == null) {
                    Stats = new HaftPartStats();
                } else {
                    if (!Stats.EnableEdits) {
        ...
        private void ClientAttemptBasicWoodInBindingsInit(ICoreAPI api) {
            Dictionary<AssetLocation, List<string>> toolsWithWoodInBindingShapes = api.Assets.GetMany<List<string>>(api.Logger, "config/haft/regex/woodinbindingshapes");
        ...
        private void CacheAlternateWorkbenchSlots(ICoreClientAPI capi) {
            Dictionary<string, MeshData> slotMarkerTextures = ObjectCacheUtil.GetOrCreate(capi, HaftConstants.WorkbenchSlotShapesCache, () => {
                Shape emptySlot = capi.Assets.TryGet(new AssetLocation(HaftConstants.WorkbenchSlotMarkerShapePath + ".json"))?.ToObject<Shape>();
                ...
                markerTexSource.textures["slot"] = new CompositeTexture(new AssetLocation(HaftConstants.WorkbenchSlotMarkerEmptyPath + ".png"));
                ...
                markerTexSource.textures.Clear();
                markerTexSource.textures["slot"] = new CompositeTexture(new AssetLocation(HaftConstants.WorkbenchSlotMarkerHeadPath + ".png"));
                ...
                markerTexSource.textures.Clear();
                markerTexSource.textures["slot"] = new CompositeTexture(new AssetLocation(HaftConstants.WorkbenchSlotMarkerHandlePath + ".png"));
                ...
                markerTexSource.textures.Clear();
                markerTexSource.textures["slot"] = new CompositeTexture(new AssetLocation(HaftConstants.WorkbenchSlotMarkerBindingPath + ".png"));
                ...
                return new Dictionary<string, MeshData>() {
                    [HaftConstants.WorkbenchSlotMarkerEmptyPath] = emptyMarkerData,
                    [HaftConstants.WorkbenchSlotMarkerHeadPath] = headMarkerData,
                    [HaftConstants.WorkbenchSlotMarkerHandlePath] = handleMarkerData,
                    [HaftConstants.WorkbenchSlotMarkerBindingPath] = bindingMarkerData
                };
            });
        }
