...
namespace Haft.Client.Behaviors {
    ...
    public class ModularPartRenderingFromAttributes : CollectibleBehavior, IContainedMeshSource {
        ...
        private Dictionary<int, MultiTextureMeshRef> meshrefs => ObjectCacheUtil.GetOrCreate(api, HaftConstants.ModularPartRenderingFromAttributesMeshRefs, () => new Dictionary<int, MultiTextureMeshRef>());
        ...
        public override void OnUnloaded(ICoreAPI api) {
            if (api.ObjectCache.ContainsKey(HaftConstants.ModularPartRenderingFromAttributesMeshRefs) && meshrefs.Count > 0) { //If the Cache exists and has more then one entry, iterate through them all to dispose them and clean things up.
                foreach ((int _, MultiTextureMeshRef mesh) in meshrefs) {
                    mesh.Dispose();
                }
                ObjectCacheUtil.Delete(api, HaftConstants.ModularPartRenderingFromAttributesMeshRefs); //Clean up the cache itself!
            }
        ...
        public MeshData GenMesh(ITreeAttribute renderTree, ITextureAtlasAPI targetAtlas, Vec3f rotationInfo = null) { //This is only to handle JUST the 'PartRenderTree's, it will either render the data provided, or if the tree sent is null, just fall back to item defaults.
            ...
            if (shape == null) { //If shape cannot be found no matter what, just return nothing.
                if (item?.Shape == null) {
                    HaftModSystem.Logger.Error("Item.Shape for " + item.Code + " was null. This item might appear invisible to prevent any direct code errors.");
                } else {
                    HaftModSystem.Logger.Error("Could not find a fallback cached shape for " + item.Code + ". This item might appear invisible to prevent any direct code errors.");
                }
        ...
        private void ConstructStacksWithRecursion(List<JsonItemStack> stacks, ITreeAttribute tree, int index) { //Will likely not be able to construct full modular tools.
            ...
            foreach (string path in textureProp.values) { //Go through the whole values array, and keep adding the attributes it finds to the json string. One itemstack per each combo of attributes in the full properties.
                tree.GetOrAddTreeAttribute(HaftAttributes.ModularPartDataTree).GetPartTextureTree().SetPartTexturePathFromKey(textureProp.code, path);
