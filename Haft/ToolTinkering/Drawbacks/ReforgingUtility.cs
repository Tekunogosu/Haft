...
namespace Haft.ToolTinkering.Drawbacks {
    public static class ReforgingUtility {
        ...
        public static SmithingRecipe TryGetSmithingRecipeFromCache(ItemStack stack, ICoreAPI api) { //Pass this a tool head or any smithed tool to prepare it for a reforge!
            if (!IsBelowPercentDurability(stack, HaftModSystem.Config.PercentDamageForReforge)) { //If it is above the configurable threshold for Reforge, just return nothing.
                return null;
            }
        ...
        public static ItemStack GetWorkItemFromMetalType(ICoreAPI api, string metal) {
            var item = api.World.GetItem(new AssetLocation(HaftConstants.WorkItemCode + "-" + metal));
            if (item == null) {
                HaftModSystem.Logger.Error("Could not find this workitem! " + HaftConstants.WorkItemCode + "-" + metal);
                return null;
            }
        ...
        public static bool[,,] DamageWorkpieceForReforge(bool[,,] voxels, int totalVoxels, int targetVoxels, ICoreAPI api) {
            ...
            if (HaftModSystem.Config.DebugMessages) {
                HaftModSystem.Logger.Warning("Number of Voxels to mess with: " + numToEffect);
                HaftModSystem.Logger.Warning("FirstVoxel is " + firstVoxel.Item1 + ", " + firstVoxel.Item2 + ", " + firstVoxel.Item3);
                HaftModSystem.Logger.Warning("There are " + voxelsInLeftSlice.Length + " voxels along the slice");
            }
            ...
            while (numToEffect > 0) { //Now that the staging is set up, time to start actually iterating through and acting on the voxels!
                ...
                switch (depth) { //Depending on the return values, 0 for left, 1 for mid, 2 for right, will have to target the indidivual arrays separately.
                    case 0:
                        ...
                        if (whichSlot < 0) {
                            HaftModSystem.Logger.Error("Something happened during Generation of a Workpiece. Attempting to act on the Left Slice, but it is empty. Refusing action to prevent a crash hopefully.");
                            break;
                        }
                    ...
                    case 1:
                        ...
                        if (whichSlot < 0) {
                            HaftModSystem.Logger.Error("Something happened during Generation of a Workpiece. Attempting to act on the Middle Slice, but it is empty. Refusing action to prevent a crash hopefully.");
                            break;
                        }
                    ...
                    default:
                        ...
                        if (whichSlot < 0) {
                            HaftModSystem.Logger.Error("Something happened during Generation of a Workpiece. Attempting to act on the Right Slice, but it is empty. Could this have actually removed all bits from the rightmost slice? Refusing action to prevent a crash hopefully.");
                            break;
                        }
        ...
        public static int ChooseActionOnVoxel(ICoreAPI api) { //This will return 0 for no action, 1 for delete the voxel, 2 for move the voxel, and 3 for delete and double down.
            if (HaftModSystem.Config.NoBitLossAlternateReforgeGen) {
        ...
        public static void SerializeVoxelsToWorkPiece(ItemStack workpiece, bool[,,] voxels) {
            workpiece.Attributes.SetBytes(HaftAttributes.WorkPieceVoxels, BlockEntityAnvil.serializeVoxels(ConvertBoolToByteArray(voxels)));
        }

        public static void SetRecipeIDToWorkPiece(ItemStack workpiece, SmithingRecipe recipe) {
            workpiece.Attributes.SetInt(HaftAttributes.WorkPieceSelectedRecipeID, recipe.RecipeId);
        }
        ...
        public static bool IsPossibleMergeItem(ItemStack item, IWorldAccessor world) {
            ...
            if (world.Side.IsServer() && HaftModSystem.IgnoreCodes.Count > 0 && HaftModSystem.IgnoreCodes.Contains(item.Collectible.Code.ToString())) {
                return false;
            }
