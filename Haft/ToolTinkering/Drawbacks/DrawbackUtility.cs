        ...
        public static void ApplyRandomDrawback(IWorldAccessor world, Entity byEntity, ItemSlot itemslot, float sharpnessPercent) {
            if (HasDrawback(itemslot.Itemstack)) {
                if (HaftModSystem.Config.DebugMessages) {
                    HaftModSystem.Logger.Warning("A Tool should have had a Drawback Worsened!!!");
                }
            } else {
                if (HaftModSystem.Config.DebugMessages) {
                    HaftModSystem.Logger.Warning("A Tool should have had a Drawback applied!");
                }
            }
        }

        public static bool HasDrawback(ItemStack itemStack) {
            return itemStack.Attributes.HasAttribute(HaftAttributes.Drawback);
        }
    }
}
