    ...
    public class CollectibleBehaviorToolHandle : CollectibleBehaviorToolPartWithHealth, IModularPartRenderer {
        ...
        public override void GetHeldItemInfo(ItemSlot inSlot, StringBuilder dsc, IWorldAccessor world, bool withDebugInfo) {
            ...
            if (world.Api.Side.IsClient()) {
                var handleStats = HaftModSystem.Stats.BaseHandleStats.Get(HaftModSystem.Stats.BaseHandleParts.Get(inSlot.Itemstack.Collectible.Code.Path).handleStatTag);
                if (handleStats != null) {
                    ...
                    if (inSlot.Itemstack.HasHandleTreatmentTag()) {
                        var treatmentStats = HaftModSystem.Stats.TreatmentStats.Get(inSlot.Itemstack.GetHandleTreatmentTag());
                    ...
                    if (inSlot.Itemstack.HasHandleGripTag()) {
                        var gripStats = HaftModSystem.Stats.GripStats.Get(inSlot.Itemstack.GetHandleGripTag());
                        if (gripStats != null) {
        ...
        private static void RefuseCraft(ItemSlot outputSlot, ref EnumHandling bhHandling) {
            outputSlot.Itemstack = new ItemStack(HaftModSystem.Api.World.GetBlock(new AssetLocation("game:air")));
        ...
        public override void OnCreatedByCrafting(ItemSlot[] allInputslots, ItemSlot outputSlot, IRecipeBase byRecipe, ref EnumHandling bhHandling) {
            ...
            foreach (var slot in allInputslots) {
                if (!slot.Empty && (slot.Itemstack.Collectible.Code != HaftConstants.SandpaperCode || slot.Itemstack.Collectible.Code != HaftConstants.FirewoodCode)) {
                    if (slot.Itemstack.Collectible.Tool != null) {
                        toolSlot = slot;
                    } else if (slot.Itemstack.Collectible.Code.FirstCodePart() == HaftConstants.HandleBlankCode ||
                               slot.Itemstack.Collectible.Code.FirstCodePart() == HaftConstants.MetalHandleBlankCode ||
                               slot.Itemstack.Collectible.Code.FirstCodePart() == HaftConstants.WorkItemFirstCodePart) {
                        blankSlot = slot;
            ...
            var isSmithedBlank = blankSlot != null && blankSlot.Itemstack.Collectible.Code.FirstCodePart() == HaftConstants.WorkItemFirstCodePart;
            if ((toolSlot != null || isSmithedBlank) && blankSlot != null) {
                ...
                var isMetalBlank = blankSlot.Itemstack.Collectible.Code.FirstCodePart() == HaftConstants.MetalHandleBlankCode || isSmithedBlank;
                ...
                if (!TinkeringUtility.IsStickOrBone(outputSlot.Itemstack) && materialType != null) {
                    ...
                    if (isMetalBlank) {
                        materialTextPath = HaftConstants.HandleMetalTexturePathMinusType + materialType;
                        if (!HaftModSystem.Api.Assets.Exists(new AssetLocation(materialTextPath + ".png"))) {
                            materialTextPath = HaftConstants.IngotMetalBackupPathMinusType + materialType;
                        }
                    } else {
                        materialTextPath = HaftConstants.HandleWoodTexturePathMinusType + materialType;
                        if (!HaftModSystem.Api.Assets.Exists(new AssetLocation(materialTextPath + ".png"))) {
                    ...
                    HandlePartDefines handleStats = HaftModSystem.Stats.BaseHandleParts.TryGetValue(outputSlot.Itemstack.Collectible.Code.Path);
                    ...
                    outputSlot.Itemstack.SetPartCurrentDurability(HaftConstants.PartDurabilityBase);
                    outputSlot.Itemstack.SetPartMaxDurability(HaftConstants.PartDurabilityBase);
                ...
                if (HaftModSystem.Stats.GripParts.ContainsKey(gripOrTreatmentSlot.Itemstack.Collectible.Code.Path)) {
                    ...
                    //wooden one accepts any - see the tag comment on HaftPart.
                    ...
                    var gripPartDefine = HaftModSystem.Stats.GripParts[gripOrTreatmentSlot.Itemstack.Collectible.Code.Path];
                    var handlePartDefine = HaftModSystem.Stats.BaseHandleParts.Get(handleSlot.Itemstack.Collectible.Code.Path);
                    if (handleSlot.Itemstack.HasHandleGripTag() ||
                        ...
                        var gripWithStats = HaftModSystem.Stats.GripParts[grip.Collectible.Code.Path];
                        var gripStats = HaftModSystem.Stats.GripStats[gripWithStats.gripStatTag];
                        ITreeAttribute gripPartTree = multiPartTree.GetPartAndTransformRenderTree(HaftAttributes.ModularPartGripName);
                    ...
                    var pendingStats = pendingTreatment?.Collectible?.Code == null
                        ? null
                        : HaftModSystem.Stats.TreatmentParts.TryGetValue(pendingTreatment.Collectible.Code.Path);
                    ...
                    if (handleSlot.Itemstack.HasHandleGripTag() || (handleSlot.Itemstack.HasHandleTreatmentTag() && !buildsOnCurrentTreatment)) {
                        ...
                        ITreeAttribute handlePartTree = multiPartTree.GetPartAndTransformRenderTree(HaftAttributes.ModularPartHandleName);
                        ...
                        var treatmentStatPair = HaftModSystem.Stats.TreatmentParts.TryGetValue(treatment.Collectible.Code.Path);
                        ...
                        if (buildsOnCurrentTreatment) {
                            ...
                            if (HaftModSystem.Stats.TreatmentStats.ContainsKey(combinedTag)) {
                                resolvedStatTag = combinedTag;
                            } else {
                                HaftModSystem.Logger.Warning("No combined treatment stat '" + combinedTag + "' is configured, so applying " + treatmentStatPair.treatmentStatTag + " over " + handleSlot.Itemstack.GetHandleTreatmentTag() + " will replace it rather than build on it.");
                            }
                        }

                        var treatmentStats = HaftModSystem.Stats.TreatmentStats.TryGetValue(resolvedStatTag);
                        var handleStatPair = HaftModSystem.Stats.BaseHandleParts.TryGetValue(handleSlot.Itemstack.Collectible.Code.Path);
                        ...
                        if (treatmentStatPair.isLiquid) {
                            ...
                            HaftModSystem.Logger.Error("A treatment config is improperly set! This treatment - " + treatment.Collectible.Code + " - was marked as a liquid, but could not find a liquid container in this recipe. Will treat it as a non-liquid, but is worth fixing that!");
                            handleTextureTree.SetPartTexturePathFromKey("wood-overlay", HaftConstants.LightTreatementOverlayPath);
                        }
                        bhHandling = EnumHandling.Handled;
                    }
        ...
        public ITreeAttribute InitializeRenderTree(ITreeAttribute tree, Item item) { //This is sent the MultiPartRenderTree, but only ever used for Creative spawned items.
            var top = tree.GetOrAddTreeAttribute(HaftAttributes.ModularPartDataTree);
            top.GetPartTextureTree();
            var handleStatsPair = HaftModSystem.Stats.BaseHandleParts.TryGetValue(item.Code.Path);
        ...
        public void ResetRotationAndOffset(ItemStack handle) {
            ...
            if (tree.HasAttribute(HaftAttributes.ModularPartHandleName)) {
                var handlePartAndTransform = tree.GetPartAndTransformRenderTree(HaftAttributes.ModularPartHandleName);
            ...
            if (tree.HasAttribute(HaftAttributes.ModularPartGripName)) {
                var gripPartAndTransform = tree.GetPartAndTransformRenderTree(HaftAttributes.ModularPartGripName);
