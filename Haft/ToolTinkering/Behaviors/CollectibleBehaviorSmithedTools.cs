    ...
    public class CollectibleBehaviorSmithedTools : CollectibleBehavior {
        ...
        public override void OnCreatedByCrafting(ItemSlot[] allInputslots, ItemSlot outputSlot, IRecipeBase byRecipe, ref EnumHandling bhHandling) {
            ...
            if (HaftModSystem.Config.DebugMessages) {
                ...
                HaftModSystem.Logger.Debug("So the Smithed Durability is: " + toolDur);
                HaftModSystem.Logger.Debug("And the starting Sharpness is: " + startingSharpness);
                HaftModSystem.Logger.Debug("Finally the max Sharpness is: " + sharpness);
                if (allInputslots.Length > 0) {
                    HaftModSystem.Logger.Debug("This tool had input slots! Was it grid crafted?");
                } else {
                    HaftModSystem.Logger.Debug("This tool had no input slots! Was it smithed?");
                }
            }
        }
        ...
        public override void OnDamageItem(IWorldAccessor world, Entity byEntity, ItemSlot itemslot, ref int amount, ref EnumHandling bhHandling) {
            if (world.Side.IsServer()) { //If it's a smithed tool, only need to deal with the Sharpness, and any extra "head" damage. Head in this case is just the tool as a whole.
                ...
                if (sharpnessPer < 0.98f) {
                    if (!isBluntTool) {
                        doDamageTool = world.Rand.NextDouble() <= HaftModSystem.Config.SharpWear;
                    } else {
                        doDamageTool = world.Rand.NextDouble() <= HaftModSystem.Config.BluntWear;
                    }
                }
