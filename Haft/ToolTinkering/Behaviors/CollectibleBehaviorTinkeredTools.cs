...
namespace Haft.ToolTinkering.Behaviors {
    public class CollectibleBehaviorTinkeredTools : CollectibleBehavior {
        ...
        public override void GetHeldItemInfo(ItemSlot inSlot, StringBuilder dsc, IWorldAccessor world, bool withDebugInfo) {
            ...
            if (TinkeringUtility.ShouldNotAccessStats(inSlot) || HaftModSystem.IgnoreCodes.Count > 0 && HaftModSystem.IgnoreCodes.Contains(inSlot.Itemstack.Collectible.Code.ToString())) {
                return;
            }
            ...
            if (storedBinding != null) {
                var bindingPart = HaftModSystem.Stats.BindingParts.Get(storedBinding.Collectible.Code.Path);
                if (bindingPart != null) {
            ...
            if (didResetParts) {try {inSlot.MarkDirty();} catch (Exception e) {
                    HaftModSystem.Logger.Warning("Haft: Could not mark an itemslot dirty after fixing null tool part data (the slot likely belongs to a non-standard/virtual inventory, e.g. an auction house listing or handbook preview). This is expected and harmless in that case. Exception details: " + e.Message);
                }
            }
        }
        ...
        public override void OnCreatedByCrafting(ItemSlot[] allInputslots, ItemSlot outputSlot, IRecipeBase byRecipe, ref EnumHandling bhHandling) {
            ...
            if (headStack == null && foundToolInput == null) { //I do hope nothing hits this. At least it'll likely get the candle backup.
                HaftModSystem.Logger.Error("Somehow crafted a Tinker Tool with a recipe that could not find a head, nor tool to copy data from.\nThe tool in question is: " + outputSlot.Itemstack.Collectible.Code.ToString() + "\nAttempting to just reset the Tool Head instead. This might result in fallback data of a Candle being assigned.");
                outputSlot.Itemstack.ResetNullHead(HaftModSystem.Api.World);
            ...
            if (handleStack != null) { //It probably shouldn't ever be the case it gets here and Handle is still null but hey.
                handleSuccess = HaftModSystem.Stats.BaseHandleParts.TryGetValue(handleStack.Collectible.Code.Path, out handle);
            } else {
                handleSuccess = HaftModSystem.Stats.BaseHandleParts.TryGetValue(HaftConstants.DefaultHandlePartKey, out handle); //Probably shouldn't ever run into this, but just incase something does go wrong, this might prevent a crash - and default to a stick used. Maybe if configs are not configured right this could happen!
                handleStack = new ItemStack(HaftModSystem.Api.World.GetItem(new AssetLocation(HaftConstants.DefaultHandleCode)), 1);
            }
            if (!handleSuccess) {
                handle = HaftModSystem.Stats.BaseHandleParts.First().Value;
            }
            ...
            if (bindingStack != null) { //If there is a binding used, then get that one.
                ...
                binding = HaftModSystem.Stats.BindingParts.Get(bindingStack.Collectible.Code.Path);
            } else {
                binding = HaftModSystem.Stats.BindingParts.Get(HaftConstants.DefaultBindingPartKey);
            }
            var handleStats = HaftModSystem.Stats.BaseHandleStats.Get(handle.handleStatTag);
            //A null binding part means none was used, which is a valid way to build a tool rather than missing data.
            var bindingStats = HaftModSystem.Stats.BindingStats.Get(binding == null ? HaftConstants.DefaultBindingStatKey : binding.bindingStatTag);
            var stats = HaftPartStatsHelpers.ResolveHandleStats(handleStats, handleStack, bindingStats);
            ...
            if (currentHeadSharpPer < 0) {
                if (isHeadMetal) {
                    currentHeadSharpPer = HaftConstants.StartingSharpnessMult;
                } else {
                    currentHeadSharpPer = HaftConstants.NonMetalStartingSharpnessMult;
                }
            }
            ...
            var speedBonus = HaftPartStatsHelpers.CalculateSpeedBonus(stats);
            var gripChanceDamage = HaftPartStatsHelpers.CalculateGripChanceToDamage(stats);
            ...
            if (HaftModSystem.Config.DebugMessages) {
                ...
                HaftModSystem.Logger.Debug("Handle Current Durability: " + (int)(handleDur * currentHandlePer));
                HaftModSystem.Logger.Debug("Binding Durability: " + bindingDur);
                HaftModSystem.Logger.Debug("Speed Bonus: " + speedBonus);
                HaftModSystem.Logger.Debug("Chance To Damage: " + gripChanceDamage);
            }
        ...
        private void HandleExtraModCompat(ItemSlot[] allInputslots, ItemSlot outputSlot) {
            if (HaftModSystem.Api.ModLoader.IsModEnabled("xskills")) { //Copy over the Quality Attribute (if it exists) onto the output item so that the GetMaxDurability will account for it here!
                HandleXSkillsCompat(allInputslots, outputSlot);
            }

            if (HaftModSystem.Api.ModLoader.IsModEnabled("itemrarity")) {
        ...
        public override void OnDamageItem(IWorldAccessor world, Entity byEntity, ItemSlot itemslot, ref int amount, ref EnumHandling bhHandling) {
            if (world.Side.IsServer() && (HaftModSystem.IgnoreCodes.Count == 0 || !HaftModSystem.IgnoreCodes.Contains(itemslot.Itemstack.Collectible.Code.ToString()))) { //Lets just make sure it isn't on the ignore list.
                ...
                if (doDamageHead && (doubleHeadDamage || (!isBluntTool && world.Rand.NextDouble() <= HaftModSystem.Config.SharpWear) || world.Rand.NextDouble() <= HaftModSystem.Config.BluntWear)) { //If this Tinkered Tool is also marked as a blunted tool, then apply the much much smaller chance to damage it. Damage the other parts though!
                ...
                //so the vanilla durability logged here is only ever changed by Haft itself.
                if (HaftModSystem.Config.DebugMessages) {
                    HaftModSystem.Logger.Debug("[BreakTrace] OnDamageItem on " + itemStack.Collectible.Code + ", amount " + amount + ", slot " + itemslot.GetType().Name + ", inventory " + (itemslot.Inventory == null ? "NULL (projectile or dummy)" : itemslot.Inventory.GetType().Name));
                    HaftModSystem.Logger.Debug("[BreakTrace] post-damage head/handle/binding: " + remainingHeadDur + " / " + remainingHandleDur + " / " + remainingBindingDur + ", headBroke: " + headBroke + ", anyPartBroke: " + anyPartBroke);
                    HaftModSystem.Logger.Debug("[BreakTrace] vanilla durability reads " + itemStack.Collectible.GetRemainingDurability(itemStack) + " (never decremented by vanilla here - PreventDefault is set)");
                }
