...
namespace Haft.ToolTinkering.Items {
    ...
    public class ItemWorkableNugget : ItemNugget, IAnvilWorkable {
        ...
        public static int AddVoxels(ICoreAPI api, ref byte[,,] voxels) {
            ...
            if (api.World.Rand.NextDouble() <= HaftModSystem.Config.ExtraBitVoxelChance) {
                voxelsToAdd += 1;
            }
        ...
        public string GetMetalType() {
            ...
            if (HaftModSystem.Config.DebugMessages && smeltedCode == null) {
                HaftModSystem.Logger.Error("Something is being given the Workable Nugget Class but has no combustible props. This will cause the item to not function like a proper workable item. The item in question is: " + Code);
            }
            return smeltedCode;
        }
    }
