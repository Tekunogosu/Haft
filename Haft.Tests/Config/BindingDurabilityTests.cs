...
public class BindingDurabilityTests {
    ...
    public void BindingDurabilityDoesNotVaryWithTheToolItIsOn() {
        ...
        var first = HaftPartStatsHelpers.CalculateBindingDurability(StatFixtures.Carved(), nails, StatFixtures.Oak());
        var second = HaftPartStatsHelpers.CalculateBindingDurability(StatFixtures.Carved(), nails, StatFixtures.Oak());
    ...
    public void NonMetalBindingIgnoresTheMaterialNailBonus() {
        ...
        var onNeutral = HaftPartStatsHelpers.CalculateBindingDurability(
            StatFixtures.Carved(), wrap, StatFixtures.NeutralMaterial());
        var onGrippy = HaftPartStatsHelpers.CalculateBindingDurability(
    ...
    public void APorousMaterialWeakensANailedBinding() {
        ...
        var durability = HaftPartStatsHelpers.CalculateBindingDurability(StatFixtures.Carved(), nails, kapok);
        var neutral = HaftPartStatsHelpers.CalculateBindingDurability(
            StatFixtures.Carved(), nails, StatFixtures.NeutralMaterial());
    ...
    public void ABetterHandleSupportsTheBindingMore() {
        ...
        var onCarved = HaftPartStatsHelpers.CalculateBindingDurability(StatFixtures.Carved(), wrap, StatFixtures.Oak());
        var onProfessional = HaftPartStatsHelpers.CalculateBindingDurability(StatFixtures.Professional(), wrap, StatFixtures.Oak());

        Assert.True(onProfessional > onCarved);
        Assert.Equal(HaftConstants.PartDurabilityBase * 1.2f, onCarved, Tolerance);
        Assert.Equal(HaftConstants.PartDurabilityBase * 1.4f, onProfessional, Tolerance);
    }
    ...
    public void UnbonusedBindingLandsOnTheFlatBase() {
        var durability = HaftPartStatsHelpers.CalculateBindingDurability(
